...
import {
    ...
    NIGHT_TIME_BRIGHTNESS_SENSOR_VALUE_MAX,
    BRIGHTNESS_MIN,
    BRIGHTNESS_MAX,
} from 'src/constants';
...
socket.on('brightness', (value) => {
    ...
    const sensorValueMax = isNight ? NIGHT_TIME_BRIGHTNESS_SENSOR_VALUE_MAX : BRIGHTNESS_SENSOR_VALUE_MAX;

    const brightness = Math.round(remapValue({
        value,
        inMin: BRIGHTNESS_SENSOR_VALUE_MIN,
        inMax: sensorValueMax,
